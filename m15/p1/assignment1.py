...
class CiphertextMessage(Message):
    ...
    def num_validwords(self, decrypted_string):
        '''Finding the valid word count'''
        validword_count = 0
        for word in decrypted_string.split(' '):
            if is_word(self.valid_words, word):
                validword_count += 1
        return validword_count

    def decryptmessage_v(self):
        '''
        ...
        decryptmessage_v = []
        for _ in range(0, 27):
            decryptmessage_v.append(self.apply_shift(1))
        return (24 - decryptmessage_v.index(max(decryptmessage_v, key=self.num_validwords)) + 1,\
            max(decryptmessage_v, key=self.num_validwords))

### DO NOT MODIFY THIS METHOD ###
def main():
    ''' This method is provided to handle testcases'''
    ciphertext = CiphertextMessage(input())
    print(ciphertext.decryptmessage_v())
